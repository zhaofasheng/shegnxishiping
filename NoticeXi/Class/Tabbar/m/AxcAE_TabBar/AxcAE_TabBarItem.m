...
- (void)itemDidLayoutControl{
    ...
    switch (self.itemModel.itemLayoutStyle) {
        ...
        case AxcAE_TabBarItemLayoutStyleTitle:{                         // 单标题占满全部
            ...
            CGFloat width = GET_STRWIDTH(@"免费", 16, 20);
            self.backLine.frame = CGRectMake((self.frame.size.width-width)/2, (self.frame.size.height-16)/2+12, width, 8);
...
- (void)setItemModel:(AxcAE_TabBarConfigModel *)itemModel{
    ...
    self.backLine = _itemModel.backLine;
...
- (void)setIsSelect:(BOOL)isSelect{
    ...
    if (_isSelect) { // 是选中
        ...
        self.titleLabel.font = XGEightBoldFontSize;
        ...
        self.titleLabel.font = XGSIXBoldFontSize;
...
- (UIView *)backLine{
    if (!_backLine) {
        ...
        _backLine.layer.cornerRadius = 4;
        CGFloat width = GET_STRWIDTH(@"免费", 16, 20);
        CAGradientLayer *gradientLayer = [[CAGradientLayer alloc] init];
        
        gradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"#1FC7FF"].CGColor,(__bridge id)[[UIColor colorWithHexString:@"#1FC7FF"] colorWithAlphaComponent:0].CGColor];
        gradientLayer.startPoint = CGPointMake(0, 1);
        gradientLayer.endPoint = CGPointMake(1, 1);
        gradientLayer.frame = CGRectMake(0, 0, width, 8);
  
        [_backLine.layer addSublayer:gradientLayer];
    }
    return _backLine;
}
