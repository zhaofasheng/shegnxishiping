...
@interface SXShopSayComModel : NSObject
...
@property (nonatomic, strong) SXShopSayComModel *firstReplyModel;//回复数组 非定位列表和正常列表的时候都可能有这个

@property (nonatomic, strong) NSDictionary *comment;//第一条评论数据 定位的时候才存在
@property (nonatomic, strong) SXShopSayComModel *firstCommentModel;//第一条评论数据 定位的时候才存在
