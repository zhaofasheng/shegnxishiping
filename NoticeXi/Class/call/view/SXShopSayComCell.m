...
@implementation SXShopSayComCell
...
- (void)setCommentM:(SXShopSayComModel *)commentM{
    _commentM = commentM;
    if ([commentM.fromUserInfo.userId isEqualToString:self.shopModel.user_id]) {
        self.nickNameL.text = self.shopModel.shop_name;
        [self.iconImageView sd_setImageWithURL:[NSURL URLWithString:self.shopModel.shop_avatar_url]];
    }else{
    ...
    if (![commentM.parent_id isEqualToString:commentM.post_id]) {
        ...
        if ([commentM.toUserInfo.userId isEqualToString:self.shopModel.user_id]) {
            self.replyNameL.text = self.shopModel.shop_name;
        }else{
...
- (void)refreshLikeUI:(SXShopSayComModel *)commentM{
    self.likeImageView.image = UIImageNamed(@"sx_like_noimg");
    _likeL.textColor = [UIColor colorWithHexString:@"#8A8F99"];
...
- (UILabel *)authorL{
    if (!_authorL) {
        ...
        _authorL.backgroundColor = [UIColor colorWithHexString:@"#F0F1F5"];
        [_authorL setAllCorner:15/2];
        _authorL.text = @"店主";
        _authorL.font = [UIFont systemFontOfSize:10];
        _authorL.textColor = [UIColor colorWithHexString:@"#8A8F99"];
...
- (void)likeClick{
    ...
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:[NSString stringWithFormat:@"shopDynamicCommontZan/%@/%@",self.commentM.commentId,self.commentM.is_like.boolValue?@"0":@"1"] Accept:@"application/vnd.shengxi.v5.8.7+json" isPost:YES parmaer:nil page:0 success:^(NSDictionary * _Nullable dict, BOOL success) {
        if (success) {
    ...
    switch (longPressState) {
        case UIGestureRecognizerStateBegan:{  //手势开始，对被选中cell截图，隐藏原cell
            ...
            if ([NoticeTools isManager]) {//管理员长按操作
                arr = @[@"回复",@"复制",@"管理员删除"];
            }else if ([[NoticeTools getuserId] isEqualToString:self.shopModel.user_id]){//动态发布者长按操作
                arr = @[@"回复",@"复制",@"删除"];
...
- (void)deleteComment{
    ...
    alerView.resultIndex = ^(NSInteger index) {
        if (index == 2) {
            [[DRNetWorking shareInstance] requestWithDeletePath:[NSString stringWithFormat:@"shopDynamicCommont/%@",weakSelf.commentM.commentId] Accept:@"application/vnd.shengxi.v5.8.7+json" parmaer:nil page:0 success:^(NSDictionary * _Nullable dict, BOOL success) {
                if (success) {
