...
- (void)updateDataSource{
    ...
    if (self.dataSource &&[self.dataSource respondsToSelector:@selector(originalArrayDataForTableView:)]) {
    ...
    //将新数组传出外部以更改数据源
    if (self.delegate &&[self.delegate respondsToSelector:@selector(tableView:newArrayDataForDataSource:)]) {
        [self.delegate tableView:self newArrayDataForDataSource:tempArray];
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(cellMoveFrom:toNewIndexPath:)]) {
        [self.delegate cellMoveFrom:_originalIndexPath toNewIndexPath:_relocatedIndexPath];
    }
}
...
- (void)cellSelectedAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self->_snapshot.transform = CGAffineTransformMakeScale(1.03, 1.03);
        self->_snapshot.alpha = 0.98;
        self->_snapshot.center = center;
    }];
...
- (void)didEndDraging{
    ...
    cell.hidden = NO;
    cell.alpha = 0;
    if (self.delegate && [self.delegate respondsToSelector:@selector(cellDidEndMovingInTableView:)]) {
        [self.delegate cellDidEndMovingInTableView:self];
    }
    [UIView animateWithDuration:0.2 animations:^{
        self->_snapshot.center = cell.center;
        self->_snapshot.alpha = 0;
        self->_snapshot.transform = CGAffineTransformIdentity;
        cell.alpha = 1;
    } completion:^(BOOL finished) {
        cell.hidden = NO;
        [self->_snapshot removeFromSuperview];
        self->_snapshot = nil;
        self->_originalIndexPath = nil;
        self->_relocatedIndexPath = nil;
    }];
}
