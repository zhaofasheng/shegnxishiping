...
- (void)layoutSubviews {
    ...
    if (self.cellModel.needRed) {
        if (self.tag == 0 && self.cellModel.red1.intValue) {
            self.redLabel1.hidden = NO;
            self.redLabel1.text = self.cellModel.red1;
            self.redLabel1.frame = CGRectMake(CGRectGetMaxX(self.titleLabel.frame)-3,9, (GET_STRWIDTH(self.cellModel.red1, 9, 14)+4)>=14?((GET_STRWIDTH(self.cellModel.red1, 9, 14)+4)):14, 14);
        }else if (self.tag == 1 && self.cellModel.red2.intValue){
            self.redLabel1.hidden = NO;
            self.redLabel1.text = self.cellModel.red2;
            self.redLabel1.frame = CGRectMake(CGRectGetMaxX(self.titleLabel.frame)-3,9, (GET_STRWIDTH(self.cellModel.red2, 9, 14)+4)>=14?((GET_STRWIDTH(self.cellModel.red2, 9, 14)+4)):14, 14);
        }else if (self.tag == 2 && self.cellModel.red3.intValue){
            self.redLabel1.hidden = NO;
            self.redLabel1.text = self.cellModel.red3;
            self.redLabel1.frame = CGRectMake(CGRectGetMaxX(self.titleLabel.frame)-3,9, (GET_STRWIDTH(self.cellModel.red3, 9, 14)+4)>=14?((GET_STRWIDTH(self.cellModel.red3, 9, 14)+4)):14, 14);
        }else{
...
- (UILabel *)redLabel1{
    if (!_redLabel1) {
        _redLabel1 = [[UILabel alloc] initWithFrame:CGRectMake(GET_STRWIDTH(self.titleLabel.text, 18, 18), (50-18)/2-7, 14, 14)];
        _redLabel1.layer.cornerRadius = 7;
        _redLabel1.textColor = [UIColor whiteColor];
        _redLabel1.layer.masksToBounds = YES;
        _redLabel1.backgroundColor = [UIColor colorWithHexString:@"#EE4B4E"];
        _redLabel1.textAlignment = NSTextAlignmentCenter;
        _redLabel1.font = [UIFont systemFontOfSize:9];
        [self.contentView addSubview:_redLabel1];
        _redLabel1.hidden = YES;
    }
    return _redLabel1;
}
