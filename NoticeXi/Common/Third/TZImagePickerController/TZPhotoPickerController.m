...
- (void)viewDidLoad {
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithTitle:@"相簿" style:UIBarButtonItemStylePlain target:self action:@selector(navLeftBarButtonClick)];
...
- (void)configBottomToolBar {
    ...
    [_previewButton setTitleColor:[UIColor colorWithHexString:@"#1FC7FF"] forState:UIControlStateNormal];
    [_previewButton setTitleColor:[UIColor colorWithHexString:@"#8A8F99"] forState:UIControlStateDisabled];
    ...
    _divideLine.backgroundColor = [UIColor whiteColor];
...
- (void)viewDidLayoutSubviews {
    ...
    _doneButton.frame = CGRectMake((self.view.tz_width -  _doneButton.tz_width)/2, 0, MAX(44, _doneButton.tz_width), 50);
    _numberImageView.frame = CGRectMake(_doneButton.tz_right, 13, 24, 24);
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            TZImageRequestOperation *operation = [[TZImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                ...
                if (havenotShowAlert && alertView) {
                    ...
                    if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingTZAssetModel:)]) {
                        [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didFinishPickingTZAssetModel:model];
                    }
                } else {
                    [self didGetAllPhotos:photos assets:assets infoArr:infoArr];
                }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader] && indexPath.section == 0) {          // Header视图
        ...
        if (@available(iOS 14, *)) {
            ...
            if (photoAuthorStatus == PHAuthorizationStatusLimited){
                headerView.titleL.text = @"当前只能访问部分照片，建议点击这里允许访问所有照片";
                headerView.titleL.textColor = [UIColor colorWithHexString:@"#EE4B4E"];
                headerView.openSetingBlock = ^(BOOL open) {
