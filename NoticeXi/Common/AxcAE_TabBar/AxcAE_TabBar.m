...
@property (nonatomic, assign) BOOL noAmition;//某次点击不需要动画
...
//            [item addGestureRecognizer:doubleRecognizer];
        }
...
- (void)longPressGestureRecognized:(id)sender{
    ...
    if (longPressState == UIGestureRecognizerStateBegan) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(longTap)]) {
            [self.delegate longTap];
        }
    }
}

//双击
- (void)dubleTap:(UITapGestureRecognizer *)sender{
...
- (void)switch_tabBarItemIndex:(NSInteger )index WithAnimation:(BOOL )animation{
    ...
    if (item.itemModel.isRepeatClick) { // 允许重复点击触发动画
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(axcAE_TabBarDouble:selectIndex:)]) {
            [self.delegate axcAE_TabBarDouble:weakSelf selectIndex:index];
        }
...
- (void)setSelectIndex:(NSInteger)selectIndex{
    ...
    [self switch_tabBarItemIndex:_selectIndex WithAnimation:YES];
}
