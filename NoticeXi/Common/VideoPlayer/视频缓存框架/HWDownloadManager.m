...
- (void)downloadwithModel:(HWDownloadModel *)model
{
    ...
    if (downloadModel.resumeData) {
        ...
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:downloadModel.url]];
        //设置编解码，不然无法获取文件总大小
...
- (void)deleteTaskAndCache:(HWDownloadModel *)model
{
    ...
    [SXTools setSaveCurrentPlayTime:0 vid:model.vid];
    [SXTools setCurrentPlayTime:0 vid:model.vid];
    // 删除本地缓存、数据库数据
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[NSFileManager defaultManager] removeItemAtPath:model.localPath error:nil];
        [SXTools deleteImageWithPath:[NSString stringWithFormat:@"%@_%@.%@",model.vid,[NoticeTools getuserId],[model.videoCover pathExtension]]];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location
{
    ...
    if (error) DRLog(@"下载完成，移动文件发生错误：%@", error);
}
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    //下载完成时候看看是否还有等待下载的视频，有就继续下一个视频的下载
