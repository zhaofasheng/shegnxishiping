...
{
    // 数据库文件路径
    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"HWDownloadVideoCaches.sqlite"]];
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        // 创表
        BOOL result = [db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_videoCaches (id integer PRIMARY KEY AUTOINCREMENT, video_len text, screen text, nickName text, vid text, fileName text, userId text, url text, videoCover text, resumeData blob, totalFileSize integer, tmpFileSize integer, state integer, progress float, lastSpeedTime double, intervalFileSize integer, lastStateTime integer)"];
        if (result) {
//            HWLog(@"视频缓存数据表创建成功");
        }else {
            DRLog(@"视频缓存数据表创建失败");
...
{
    [_dbQueue inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"INSERT INTO t_videoCaches (video_len, screen, nickName, vid, fileName, userId, url, videoCover, resumeData, totalFileSize, tmpFileSize, state, progress, lastSpeedTime, intervalFileSize, lastStateTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",model.video_len, model.screen, model.nickName, model.vid, model.fileName, model.userId, model.url, model.videoCover, model.resumeData, [NSNumber numberWithInteger:model.totalFileSize], [NSNumber numberWithInteger:model.tmpFileSize], [NSNumber numberWithInteger:model.state], [NSNumber numberWithFloat:model.progress], [NSNumber numberWithDouble:0], [NSNumber numberWithInteger:0], [NSNumber numberWithInteger:0]];
        if (result) {
            DRLog(@"插入成功：%@--%@", model.fileName,model.nickName);
        }else {
            DRLog(@"插入失败：%@", model.fileName);
        }
    }];
}
