...
@implementation NoticeSongDetail
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [self.searchBtn setTitle:[NoticeTools getLocalType]?@"  search":@"  搜索歌曲" forState:UIControlStateNormal];
        ...
        label.text = [NoticeTools getLocalStrWith:@"music.hot"];
        ...
        self.movieTableView.frame = CGRectMake(20,45+66,DR_SCREEN_WIDTH-40, 155-35);
...
- (void)requestList{
    ...
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:@"resources/3?sortType=2&recentDay=30" Accept:@"application/vnd.shengxi.v3.2+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        if (success) {
...
- (void)allClick{
    ...
    NoticeHotSongController *ctl = [[NoticeHotSongController alloc] init];
    [nav.topViewController.navigationController pushViewController:ctl animated:YES];
}
