...
+ (void)registerLocalNotification API_AVAILABLE(ios(10.0)) {
    ...
    [center requestAuthorizationWithOptions:(UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert) completionHandler:^(BOOL granted, NSError * _Nullable error) {
        if (!error) {
            DRLog(@"request authorization succeeded!");
        }
    }];
    ...
    UNNotificationCategory *category = [UNNotificationCategory categoryWithIdentifier:categryLaterIdf actions:@[action1,action4] intentIdentifiers:@[] options:UNNotificationCategoryOptionNone];
...
+ (UNMutableNotificationContent *)contentWithTitle:(NSString *)title subTitle:(NSString *)subTitle body:(NSString *)body {
    ...
    //title、body不能为空
    NSString *titleStr = title.length > 0 ? title : @"";
    NSString *bodyStr = body.length > 0 ? body : @"";
    UNMutableNotificationContent *content = [[UNMutableNotificationContent alloc] init];
    content.title = titleStr;
    content.subtitle = subTitle;
    content.body = bodyStr;
    content.sound = [UNNotificationSound defaultSound];
//    NSString *imgViewPath = [[NSBundle mainBundle] pathForResource:@"hotM_01" ofType:@"caf"];
//    UNNotificationAttachment *att = [UNNotificationAttachment attachmentWithIdentifier:@"haha" URL:[NSURL fileURLWithPath:imgViewPath] options:nil error:nil];
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler  API_AVAILABLE(ios(10.0)) {
    ...
    if (pushM.type.intValue == 11) {
        NoticePinBiView *pinV = [[NoticePinBiView alloc] initWithWarnTostViewContent:pushM.title];
        [pinV showTostView];
        return;
    }
    DRLog(@"推送2\n%@",userInfo);
    DRLog(@"应用在前台时候接收到本地推送通知、远程推送通知调用此方法%@",notification.request.identifier);

    if ([notification.request.identifier isEqualToString:@"shengxinaozhong"] || [notification.request.identifier containsString:@"shengxinaozhong"]) {
        AppDelegate *app = (AppDelegate *)[UIApplication sharedApplication].delegate;
        app.isPlayingNoticeVice = NO;
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];//扬声器模式
        [[NSNotificationCenter defaultCenter] postNotificationName:@"NOTICATIONCLOCK" object:nil userInfo:nil];
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler  API_AVAILABLE(ios(10.0)){
    DRLog(@"应用程序在后台，用户通过点击本地推送、远程推送进入app时调用此方法%@",response);
...
-(void)handCommnet:(UNNotificationResponse *)response API_AVAILABLE(ios(10.0)) {
    ...
    if (date) {
        ...
        NSString *identifer = [NSString stringWithFormat:@"shengxinaozhong%@",[format stringFromDate:[NSDate date]]];
