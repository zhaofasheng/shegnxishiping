//
//  NoticeSameLikeBookController.m
//  NoticeXi
//
...
@interface NoticeSameLikeBookController ()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeSameLikeBookController
...
- (void)viewDidLoad {
    ...
    label.text = [NoticeTools isSimpleLau]?[NSString stringWithFormat:@"以下你喜欢的书籍 %@ 也喜欢",self.name]: [NSString stringWithFormat:@"以下你喜欢的書籍 %@ 也喜欢",self.name];
    ...
    self.navigationItem.title = [NoticeTools isSimpleLau] ?@"共同喜欢的书":@"共同喜歡的書";
...
- (void)createRefesh{
    ...
    __weak NoticeSameLikeBookController *ctl = self;
...
- (void)requestData{
    NSString *url = nil;
    if (self.isDown) {
        url = [NSString stringWithFormat:@"hobby/common/2/users/%@",self.withUserId];
    }else{
        if (self.lastId) {
            url = [NSString stringWithFormat:@"hobby/common/2/users/%@?lastId=%@",self.withUserId,self.lastId];
        }else{
            url = [NSString stringWithFormat:@"hobby/common/2/users/%@",self.withUserId];
        }
    }
