//
//  NoticeSameLikeSongController.m
//  NoticeXi
//
...
@interface NoticeSameLikeSongController ()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeSameLikeSongController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(15,15, DR_SCREEN_WIDTH-90,14)];
    label.font = FOURTHTEENTEXTFONTSIZE;
    label.textColor = GetColorWithName(VMainTextColor);
    label.text = [NoticeTools isSimpleLau]?[NSString stringWithFormat:@"以下你最爱的歌也是 %@ 最爱的歌",self.name]: [NSString stringWithFormat:@"以下你最愛的歌也是 %@ 最愛的歌",self.name];
    [self.view addSubview:label];
    [self.view bringSubviewToFront:label];
    
    self.dataArr = [NSMutableArray new];
    
    self.tableView.frame = CGRectMake(0, CGRectGetMaxY(label.frame), DR_SCREEN_WIDTH, DR_SCREEN_HEIGHT-14-NAVIGATION_BAR_HEIGHT-15);
    
    self.view.backgroundColor = GetColorWithName(VBackColor);
    self.tableView.backgroundColor = self.view.backgroundColor;
    self.navigationItem.title = [NoticeTools isSimpleLau] ?@"共同最爱的歌":@"共同最愛的歌";
...
- (void)createRefesh{
    ...
    __weak NoticeSameLikeSongController *ctl = self;
...
- (void)requestData{
    NSString *url = nil;
    if (self.isDown) {
        url = [NSString stringWithFormat:@"hobby/common/3/users/%@",self.withUserId];
    }else{
        if (self.lastId) {
            url = [NSString stringWithFormat:@"hobby/common/3/users/%@?lastId=%@",self.withUserId,self.lastId];
        }else{
            url = [NSString stringWithFormat:@"hobby/common/3/users/%@",self.withUserId];
        }
    }
...
- (void)addNumbers:(NoticeVoiceListModel *)choiceModel{
    ...
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:url Accept:nil isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        ...
        if (success) {
            ...
            model.played_num = [NSString stringWithFormat:@"%d",(int)model.played_num.integerValue+1];
            [self.tableView reloadData];
