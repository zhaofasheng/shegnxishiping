...
@interface NoticeSearchSongController ()<UITextFieldDelegate,NoticeTopiceCancelDelegate>
@property (nonatomic, strong) UITextField *topicField;
@property (nonatomic, strong) NSMutableArray *locaArr;
@property (nonatomic, strong) UIButton *moreButton;
@end

@implementation NoticeSearchSongController
...
- (void)viewDidLoad {
    ...
    self.topicField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[NoticeTools getLocalStrWith:@"music.search"] attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#ACB3BF"]}];
...
- (void)lookMore{
    ...
    if (!_isMore) {
        ...
        [NoticeTools saveSongArr:self.locaArr];
        [self.moreButton setTitle:GETTEXTWITE(@"topic.history") forState:UIControlStateNormal];
        _isMore = YES;
    }
    _isMore = NO;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NoticeSearchSongResultController *ctl = [[NoticeSearchSongResultController alloc] init];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.topicField becomeFirstResponder];
    self.locaArr = [NoticeTools getSongArr];
    [self.tableView reloadData];
