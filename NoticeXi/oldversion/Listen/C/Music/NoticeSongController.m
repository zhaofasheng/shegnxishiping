...
@interface NoticeSongController ()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeSongController
- (UIView *)listView {
    return self.view;
}
...
- (void)createRefesh{
    ...
    __weak NoticeSongController *ctl = self;
    ...
    if (self.isDown) {
        ...
        if (self.isNew) {
            if ([NoticeTools voiceType]) {
                url = [NSString stringWithFormat:@"club/3/voices?contentType=%ld",[NoticeTools voiceType]];
            }else{
                url = @"club/3/voices";
            }
            ...
            if ([NoticeTools voiceType]) {
                [NSString stringWithFormat:@"voices/subscription?resourceType=3?contentType=%ld",[NoticeTools voiceType]];
            }else{
                url = @"voices/subscription?resourceType=3";
            }
        ...
        if (self.isNew) {
            if (self.lastId) {
                if ([NoticeTools voiceType]) {
                    url = [NSString stringWithFormat:@"club/3/voices?lastId=%@&contentType=%ld",self.lastId,[NoticeTools voiceType]];
                }else{
            ...
            if ([NoticeTools voiceType]) {
                url = [NSString stringWithFormat:@"voices/subscription?resourceType=3&lastId=%@&contentType=%ld",self.glastId,[NoticeTools voiceType]];
            }else{
                url = [NSString stringWithFormat:@"voices/subscription?resourceType=3&lastId=%@",self.glastId];
            }
...
- (void)tapTost{
    NoticePinBiView *tostV = [[NoticePinBiView alloc] initWithLeaderBook:3];
    [tostV showTostView];
}
@end
