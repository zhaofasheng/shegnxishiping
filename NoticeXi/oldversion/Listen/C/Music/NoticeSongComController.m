...
@interface NoticeSongComController ()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeSongComController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title =self.userId ?[NoticeTools getLocalStrWith:@"movie.hisbanb"] : [NoticeTools getLocalStrWith:@"movie.mybanb"];
...
- (void)createRefesh{
    ...
    __weak NoticeSongComController *ctl = self;
...
- (void)requestData{
    NSString *url = nil;
    if (self.isDown) {
        url = [NSString stringWithFormat:@"review/users/%@/3/%@",self.userId?self.userId:[[NoticeSaveModel getUserInfo] user_id],self.song.albumId];
    }else{
        if (self.lastId) {
            url = [NSString stringWithFormat:@"review/users/%@/3/%@?lastId=%@",self.userId?self.userId:[[NoticeSaveModel getUserInfo] user_id],self.song.albumId,self.lastId];
        }else{
            [NSString stringWithFormat:@"review/users/%@/3/%@",self.userId?self.userId:[[NoticeSaveModel getUserInfo] user_id],self.song.albumId];
        }
    }
