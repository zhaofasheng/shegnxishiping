...
@interface NoticeMovieViewController ()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeMovieViewController
- (UIView *)listView {
    return self.view;
}
...
- (void)viewDidLoad {
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, DR_SCREEN_WIDTH-100, 33)];
    button.backgroundColor = [UIColor colorWithHexString:[NoticeTools isWhiteTheme]?@"#EFEFEF":@"#25253D"];
    button.layer.cornerRadius = 35/2;
    button.layer.masksToBounds = YES;
    button.titleLabel.font = FOURTHTEENTEXTFONTSIZE;
    [button setTitleColor:[NoticeTools isWhiteTheme] ? [UIColor colorWithHexString:@"#999999"] : [UIColor colorWithHexString:@"#72727F"] forState:UIControlStateNormal];
    [button setTitle:[NoticeTools getTextWithSim:[NoticeTools getLocalStrWith:@"movie.sdinay"] fantText:@"搜電影"] forState:UIControlStateNormal];
...
- (void)createRefesh{
    ...
    __weak NoticeMovieViewController *ctl = self;
    ...
    if (self.isDown) {
        ...
        if (self.isNew) {
            if ([NoticeTools voiceType]) {
                url = [NSString stringWithFormat:@"club/1/voices?contentType=%ld",[NoticeTools voiceType]];
            }else{
                url = @"club/1/voices";
            }
            ...
            if ([NoticeTools voiceType]) {
                [NSString stringWithFormat:@"voices/subscription?resourceType=1?contentType=%ld",[NoticeTools voiceType]];
            }else{
                url = @"voices/subscription?resourceType=1";
            }
        ...
        if (self.isNew) {
            if (self.lastId) {
                if ([NoticeTools voiceType]) {
                    url = [NSString stringWithFormat:@"club/1/voices?lastId=%@&contentType=%ld",self.lastId,[NoticeTools voiceType]];
                }else{
            ...
            if ([NoticeTools voiceType]) {
                url = [NSString stringWithFormat:@"voices/subscription?resourceType=1&lastId=%@&contentType=%ld",self.glastId,[NoticeTools voiceType]];
            }else{
                url = [NSString stringWithFormat:@"voices/subscription?resourceType=1&lastId=%@",self.glastId];
            }
...
- (NoticeNoDataView *)noDataView{
    if (!_noDataView) {//
        ...
        _noDataView.titleImageV.frame = CGRectMake((DR_SCREEN_WIDTH-120)/2,31,120, 120);
...
- (void)tapTost{
    NoticePinBiView *tostV = [[NoticePinBiView alloc] initWithLeaderBook:1];
    [tostV showTostView];
}
