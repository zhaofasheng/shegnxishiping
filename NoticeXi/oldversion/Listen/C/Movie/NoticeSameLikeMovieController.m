//
//  NoticeSameLikeMovieController.m
//  NoticeXi
//
...
@interface NoticeSameLikeMovieController()<NoticeVoiceListClickDelegate,LCActionSheetDelegate>
...
@implementation NoticeSameLikeMovieController
...
- (void)viewDidLoad {
    ...
    label.text = [NSString stringWithFormat:@"以下你喜欢的电影 %@ 也喜欢",self.name];
    ...
    self.navigationItem.title = [NoticeTools isSimpleLau] ?@"共同喜欢的电影":@"共同喜歡的電影";
...
- (void)createRefesh{
    ...
    __weak NoticeSameLikeMovieController *ctl = self;
...
- (void)requestData{
    NSString *url = nil;
    if (self.isDown) {
        url = [NSString stringWithFormat:@"hobby/common/1/users/%@",self.withUserId];
    }else{
        if (self.lastId) {
            url = [NSString stringWithFormat:@"hobby/common/1/users/%@?lastId=%@",self.withUserId,self.lastId];
        }else{
            url = [NSString stringWithFormat:@"hobby/common/1/users/%@",self.withUserId];
        }
    }
