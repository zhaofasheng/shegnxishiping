//
//  NoticeSearchMovieViewController.m
//  NoticeXi
//
...
@interface NoticeSearchMovieViewController ()<UITextFieldDelegate,NoticeTopiceCancelDelegate>
@property (nonatomic, strong) UITextField *topicField;
@property (nonatomic, strong) NSMutableArray *locaArr;
@property (nonatomic, strong) UIButton *moreButton;
@end

@implementation NoticeSearchMovieViewController
{
    BOOL _isMore;
}
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.topicField becomeFirstResponder];
     self.locaArr = [NoticeTools getMovieArr];
...
- (void)viewDidLoad {
    ...
    self.topicField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[NoticeTools getLocalStrWith:@"movie.searchhotmv"] attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#ACB3BF"]}];
...
- (void)lookMore{
    ...
    if (!_isMore) {
        [self.locaArr removeAllObjects];
         [NoticeTools saveMoveArr:self.locaArr];
        [self.moreButton setTitle:GETTEXTWITE(@"topic.history") forState:UIControlStateNormal];
        _isMore = YES;
    }
    _isMore = NO;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NoticeSearchMovieResultViewController *ctl = [[NoticeSearchMovieResultViewController alloc] init];
