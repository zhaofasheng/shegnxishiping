...
//    NSInteger hourInt = [dateString substringWithRange:NSMakeRange(8, 2)].integerValue;
    NSInteger minuteInt = [dateString substringWithRange:NSMakeRange(10, 2)].integerValue;
    
    minuteInt = (minuteInt % 5) ? (minuteInt / 5) * 5 : (minuteInt / 5 + 1) * 5;
    
    
    resultTimeStr=[NSString stringWithFormat:@"%@-%02ld-%02ld %@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[NSString stringWithFormat:@"%02ld",minuteInt]];
    
//    NSLog(@"resultTimeStr-resultTimeStr:%@",resultTimeStr);
...
//    [self setSecondScrollView];
    ...
    [send setTitle:[NoticeTools getLocalStrWith:@"main.sure"] forState:UIControlStateNormal];
...
{
    yearScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(0, 0, 81.6, 190.0)];
...
{
    monthScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(81.6, 0, 49.1, 190.0)];
...
{
    dayScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(130.7, 0, 54.6, 190.0)];
...
{
    hourScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(185.3, 0, 47.6, 190.0)];
...
{
    minuteScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(232.9, 0, 45.6, 190.0)];
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    {
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"%02ld",index * 5];
        }
        else
            l.text = [NSString stringWithFormat:@"%ld",index * 5];
    }
    else
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"0%ld",index];
...
- (NSInteger)setNowTimeShow:(NSInteger)timeType
{
    ...
    switch (timeType) {
        ...
        case 4:
        {
            ...
            NSInteger minuteInt = minuteString.integerValue % 5 ? (minuteString.integerValue / 5) : (minuteString.integerValue / 5 + 1);
//            NSLog(@"----- %ld",minuteString.integerValue);
            return minuteInt;
        }
            break;
        case 5:
        {
            ...
            NSString *secondString = [dateString substringWithRange:range];
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSInteger yearInt = ABS(yearLabel.tag + 1999);
    NSInteger monthInt = ABS(monthLabel.tag);
    NSInteger dayInt = ABS(dayLabel.tag);
    NSInteger hourInt = ABS(hourLabel.tag - 1);
    NSInteger minuteInt = ABS(minuteLabel.tag - 1);
//    (minuteLabel.tag - 1) < 0 ? -(minuteLabel.tag - 1) : (minuteLabel.tag - 1);
    NSInteger secondInt = ABS(secondLabel.tag - 1);
...
//     minuteInt = (minuteInt % 5) ? (minuteInt / 5) * 5 : (minuteInt / 5 + 1) * 5;
    ...
    resultTimeStr=[NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",yearInt,monthInt,dayInt,hourInt,minuteInt * 5];
    
    
//    NSLog(@"resultTimeStr - %@",resultTimeStr);
