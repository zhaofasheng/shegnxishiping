...
- (void)xwp_gestureBegan:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    UIGraphicsBeginImageContextWithOptions(cell.bounds.size, 0.0f, 0);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetFillColorWithColor(context, [GetColorWithName(VlistColor) CGColor]);
    CGContextFillRect(context, CGRectMake(0, 0, cell.bounds.size.width, cell.bounds.size.height));
...
- (void)xwp_gestureEndOrCancle:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self->_tempMoveCell.center = self->_orignalCenter;
    } completion:^(BOOL finished) {
        [self xwp_stopShakeAllCell];
        [self->_tempMoveCell removeFromSuperview];
        cell.hidden = NO;
        self->_orignalCell.hidden = NO;
...
- (void)xwp_stopShakeAllCell{
    if (!_shakeWhenMoveing || _isEditing) {
        return;
    }
    NSArray *cells = [self visibleCells];
    for (UICollectionViewCell *cell in cells) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if (_isEditing || _isPanning) {
        [self xwp_shakeAllCell];
    }else if (!_isEditing && !_isPanning){
        [self xwp_stopShakeAllCell];
    }
}
